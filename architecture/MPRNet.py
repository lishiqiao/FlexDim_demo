...
class MPRNet(nn.Module):
    def __init__(self, in_c=28, out_c=28, n_feat=28, scale_unetfeats=28, scale_orsnetfeats=28, num_cab=4, kernel_size=3, reduction=1, bias=False):
        ...
        self.conv_in = nn.Conv2d(28, in_c, kernel_size=kernel_size, padding=(kernel_size - 1) // 2,
                                 bias=bias)
    ...
    def forward(self, x3_img):
        b, c, v, h_inp, w_inp = x3_img.shape
        view = x3_img.reshape(b, c, v, h_inp * w_inp)
        b, c, h_inp, w_inp = view.shape
        # b, c, h_inp, w_inp = x3_img.shape
        hb, wb = 8, 8
        pad_h = (hb - h_inp % hb) % hb
        pad_w = (wb - w_inp % wb) % wb
        x3_img = F.pad(view, [0, pad_w, 0, pad_h], mode='reflect')
